...
import './RightSidebarBookmarks.css';

const RightSidebarBookmarks: React.FC = () => {
  const { 
    rightSidebarBookmarks, 
    createRightSidebarBookmark, 
  ...
  const handleAddBookmark = async (title: string, url: string) => {
    try {
      ...
      await createRightSidebarBookmark(updates.title, updates.url, updates.color);
      setShowAddModal(false);
    } catch (error) {
      console.error('우측 사이드바 북마크 추가 실패:', error);
    }
  };
  ...
  const handleUpdateBookmark = async (title: string, url: string) => {
    ...
    try {
      ...
      await updateRightSidebarBookmark(editingBookmark.id, updates);
      setShowAddModal(false);
      setEditingBookmark(null);
    } catch (error) {
      console.error('우측 사이드바 북마크 수정 실패:', error);
    }
  };
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('이 북마크를 삭제하시겠습니까?')) {
      try {
        await deleteRightSidebarBookmark(id);
      } catch (error) {
        console.error('우측 사이드바 북마크 삭제 실패:', error);
      }
    }
  };
  ...
  const handleDrop = async (e: React.DragEvent, targetBookmark: SidebarBookmark) => {
    ...
    const sortedBookmarks = [...rightSidebarBookmarks].sort((a, b) => a.order - b.order);
    ...
    try {
      await reorderRightSidebarBookmarks(newOrder);
    } catch (error) {
      console.error('우측 사이드바 북마크 재정렬 실패:', error);
    }
  };
  ...
  const handleRightClick = (e: React.MouseEvent, bookmark: SidebarBookmark) => {
    ...
    const x = Math.max(10, e.clientX - 180);
  ...
  const handleActionDelete = async () => {
    if (selectedBookmark) {
      ...
      if (shouldDelete) {
        await deleteRightSidebarBookmark(selectedBookmark.id);
      }
    }
    setShowActionModal(false);
    setSelectedBookmark(null);
  ...
  const handleActionColor = () => {
    if (selectedBookmark) {
      const rect = document.querySelector('.right-sidebar-bookmarks')?.getBoundingClientRect();
      const x = rect ? rect.left - 290 : modalPosition.x - 180;
  ...
  const handleColorSelect = async (color: string) => {
    if (selectedBookmark) {
      try {
        await updateRightSidebarBookmark(selectedBookmark.id, { color });
      } catch (error) {
        console.error('우측 사이드바 북마크 색상 변경 실패:', error);
      }
    }
  ...
  return (
    <div className="right-sidebar-bookmarks">
      <div className="right-sidebar-bookmarks-header">
        <h3>북마크</h3>
        <button 
          className="add-right-sidebar-bookmark-btn"
          onClick={() => {
            setEditingBookmark(null);
            setShowAddModal(true);
          }}
      ...
      <div className="right-sidebar-bookmarks-list">
        {rightSidebarBookmarks
          .sort((a, b) => a.order - b.order)
          .map((bookmark) => (
            <div
              key={bookmark.id}
              className="right-sidebar-bookmark-item"
              draggable
            ...
            >
              <div 
                className="right-sidebar-bookmark-content"
                onClick={() => handleBookmarkClick(bookmark.url)}
                onDoubleClick={() => handleEditBookmark(bookmark)}
                title={`${bookmark.title} - ${bookmark.url}`}
              >
                <div className="right-sidebar-bookmark-main">
                  <div className="right-sidebar-bookmark-title">{bookmark.title}</div>
                </div>
              </div>
            </div>
          ))}

        {rightSidebarBookmarks.length === 0 && (
          <div className="right-sidebar-empty-state">
            <p>북마크가 없습니다</p>
      ...
      {/* 액션 모달 */}
      <RightSidebarBookmarkActionModal
...
// 우측 사이드바 북마크 액션 모달
interface RightSidebarBookmarkActionModalProps {
  isOpen: boolean;
...
const RightSidebarBookmarkActionModal: React.FC<RightSidebarBookmarkActionModalProps> = ({
  isOpen,
  bookmark,
  onEdit,
  onColor,
...
// 우측 사이드바 색상 선택 모달
interface RightSidebarColorPickerProps {
  isOpen: boolean;
  bookmark: SidebarBookmark | null;
...
const RightSidebarColorPicker: React.FC<RightSidebarColorPickerProps> = ({
  isOpen,
  bookmark,
  onColorSelect,
  onCancel,
...
// 우측 사이드바 북마크 추가/편집 모달
interface RightSidebarBookmarkModalProps {
  bookmark?: SidebarBookmark | null;
  onSave: (title: string, url: string) => void;
  onCancel: () => void;
}

const RightSidebarBookmarkModal: React.FC<RightSidebarBookmarkModalProps> = ({ bookmark, onSave, onCancel }) => {
...
export default RightSidebarBookmarks;